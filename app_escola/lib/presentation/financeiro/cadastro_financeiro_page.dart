  ...
  Future<void> _salvarEGerar() async {
    ...
    if (!c.podeSalvarEGerarParcelas) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Preencha matrícula, vencimento, duração (1–120), pacote (detalhe se '
            'Outros), valores e status. Entrada + taxas não podem exceder o total.',
          ),
        ),
      );
      return;
            ...
            children: [
              ...
              Align(
                ...
                child: TextButton.icon(
                  ...
                  label: Text(
                    'Data da matrícula: '
                    '${_dataMatricula != null ? _dataFmt.format(_dataMatricula!) : "Selecionar"}',
                  ),
                ),
              ),
              const SizedBox(height: 8),
              ...
              const SizedBox(height: 12),
              Text('Turmas', style: Theme.of(context).textTheme.titleSmall),
              CheckboxListTile(
                value: _turmaTecnologia,
                onChanged: _locked
                    ? null
                    : (v) => setState(() => _turmaTecnologia = v ?? false),
              ...
              CheckboxListTile(
                ...
                onChanged: _locked
                    ? null
                    : (v) => setState(() => _turmaIngles = v ?? false),
              ...
              Align(
                ...
                child: TextButton.icon(
                  ...
                  label: Text(
                    'Primeiro vencimento: '
                    '${_dataPrimeiroVencimento != null ? _dataFmt.format(_dataPrimeiroVencimento!) : "Selecionar"}',
                  ),
                ),
              ),
              const SizedBox(height: 12),
              ...
              TextFormField(
                ...
                onChanged: (_) => setState(() {}),
                decoration: const InputDecoration(
                  labelText: 'Juros por dia útil de atraso (R\$)',
                  border: OutlineInputBorder(),
                  helperText:
                      'Multa diária sobre a mensalidade ( domingos e feriados não entram no atraso).',
                ),
              ),
              Builder(
                builder: (context) {
